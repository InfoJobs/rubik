(function (d,w) {
    ...
    function equalColumn() {
        var rowGrids = d.querySelectorAll('.grid');
        
        for( var i=0; i < rowGrids.length; i++ ) {
            
            var elCols = rowGrids[i].querySelectorAll('.col-3'),
                nHeight = 0,
                eBox;
            
            for( var j=0; j < elCols.length; j++ ) {
                elBox = elCols[j].querySelectorAll('.box-row');
                for( var k=0; k < elBox.length; k++ ) {
                  if( nHeight < elBox[k].clientHeight ) {
                    nHeight = elBox[k].clientHeight;
                  }
                }
            }
            ...
            for( var j=0; j < elCols.length; j++ ) {
              elBox = elCols[j].querySelectorAll('.box-row');
              for( var k=0; k < elBox.length; k++ ) {
                elBox[k].style.height = nHeight+'px';
              }
            }
	...
	function currentSection( current ) {
        ...
        if( w.location.pathname.indexOf( '.html' ) < 0 ) {
            d.getElementById('home').className = 'active';
        }
	}
